...
```{r, eval = FALSE}
sampleInfo <- read.csv("retina_samples.txt", sep = "\t")
sObj_List <- list()

for (i in 1:nrow(sampleInfo)){
  df <- Read10X(file.path(sampleInfo$FilePath[i], "filtered_feature_bc_matrix"))
  sObj_List[[sampleInfo$Sample[i]]] <- CreateSeuratObject(counts = df, 
                                                          project = sampleInfo$Sample[i], 
                                                          min.cells = 10, min.features = 0)
  sObj_List[[sampleInfo$Sample[i]]][["percent.mt"]] <- PercentageFeatureSet(sObj_List[[sampleInfo$Sample[i]]], 
                                                                            pattern = "^MT-")
...
for (i in 2:length(sObj_Filtered)){
