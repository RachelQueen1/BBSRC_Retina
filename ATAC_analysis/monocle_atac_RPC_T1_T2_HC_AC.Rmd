...
## RPC T1 T2 HC AC trajectory
```{r}
start_Node = "Y_3"
```

The RPC, T1, T2, HC and AC cell types were selected and Monocle 3 pseudotime analysis performe starting from `r start_Node`.  
...
RPC_T1_T2_HC_AC <- readRDS("rObjects/seurat_PT_RPC_T1_T2_HCs_ACs.rds")

cds <- as.cell_data_set(RPC_T1_T2_HC_AC)
...
T2 <- c(topMotifs[["T2"]]$motif)
HCs <- c(topMotifs[["HCs"]]$motif)
ACs <- c(topMotifs[["Gabaergic amacrine cells"]]$motif,
         topMotifs[["glycinergic amacrine cells "]]$motif)


all <- data.frame(motif = unique(c(RPCs,T1,T2,HCs,ACs)))
all$RPCs <- all$motif %in% RPCs
all$T1 <- all$motif %in% T1
all$T2 <- all$motif %in% T2
all$HCs <- all$motif %in% HCs
all$ACs <- all$motif %in% ACs
