---
title: "Monocle T1 T3 cones rods BPs"
output:
  html_document:
    df_print: paged
---
...
## T1 T3 cones rods BPs trajectory
```{r}
start_Node = "Y_26"
```

The T1, T2, HC and AC cell types were selected and Monocle 3 pseudotime analysis performe starting from `r start_Node`.  
...
T1_T3_cones_rods_BPs <- readRDS("rObjects/seurat_PT_RPC_T1_T3_cones_rods_BPs.rds")

cds <- as.cell_data_set(T1_T3_cones_rods_BPs)
...
T3 <- c(topMotifs[["T3"]]$motif)
cones <- c(topMotifs[["cones"]]$motif)
rods <- c(topMotifs[["rods"]]$motif)
BPs <- c(topMotifs[["BPs"]]$motif)


all <- data.frame(motif = unique(c(T1,T3,cones,rods,BPs)))

all$T1 <- all$motif %in% T1
all$T3 <- all$motif %in% T3
all$cones <- all$motif %in% cones
all$rods <- all$motif %in% rods
all$BPs <- all$motif %in% BPs
